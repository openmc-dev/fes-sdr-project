...
results_dict = {'mesh': []}
results_dict.update({f'det_{i}': [] for i in range(6)})
...
for nr in range(1, 6):
    for n in range(1, 11):
        ...
        sp_path = sp_path.rename(f'statepoint_mesh_{n}.h5')

        with openmc.StatePoint(sp_path) as sp:
            t = sp.tallies[tally.id]
            for i, value in enumerate(t.mean.ravel()):
                results_dict[f'det_{i}'].append(value)
        results_dict['mesh'].append((nr, nr, n))

df = pd.DataFrame(results_dict)
print(df.to_string())
df.to_pickle('results_r2s_mesh.pkl')
